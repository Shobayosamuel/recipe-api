...
def image_upload_url(recipe_id):
    """Create and return an image upload file."""
...
class PrivateRecipeAPI(TestCase):
    ...
    def setUp(self):
        self.client = APIClient()
        self.user = create_user(email='test2@example.com', password='test123')
        payload = {
    ...
    def test_partial_update(self):
        ...
        self.assertEqual(res.status_code, status.HTTP_200_OK)
        recipe.refresh_from_db()  # This reloads the database
        self.assertEqual(recipe.title, payload['title'])
        self.assertEqual(recipe.link, original_link)
        self.assertEqual(recipe.user, self.user)

    def test_full_update(self):
        """Tests full update on the recipe."""
        recipe = create_recipe(
            user=self.user,
            title='sample recipe title',
            link='https://example.com/recipe.pdf',
    ...
    def test_filter_tags(self):
        ...
        r2 = create_recipe(user=self.user, title='Efo riro')
        tag1 = Tag.objects.create(user=self.user, name='Vagan')
        tag2 = Tag.objects.create(user=self.user, name='Vagetarian')
        r1.tags.add(tag1)
        r2.tags.add(tag2)
